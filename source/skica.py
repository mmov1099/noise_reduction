...
def crop_arrays(pics):
    ...
    for pic in pics:
        ...
        min_w = min(min_w, w)
        min_h = min(min_h, h)
    return [ np.ma.array(pic[:min_h,:min_w]) for pic in pics ], min_h, min_w
...
class ICA():
    ...
    def run(self):
        ...
        self.X[~self.X.mask] = (self.X[~self.X.mask] - self.X[~self.X.mask].mean()) / self.X[~self.X.mask].std()
